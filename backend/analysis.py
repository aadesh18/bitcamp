...
data = [
  ...
  [
    ...
    '1 Bath',
    '947 Sq. Ft.',
    '4301 Massachusetts Ave NW #2007, Washington, DC 20017'
  ],
  [
    '32',
...
for i in data:
    ...
    # img link
    img_link = i[1]
    temp["image_link"] = img_link

    # beds
    beds = i[3][0]
    beds = '0' if beds == "—" else beds
    beds = beds.strip()
    temp["beds"] = beds
    
    # baths
    baths = i[4][:-5]
    baths = '0' if baths == "—" else baths
    baths = baths.strip()
    temp["baths"] = baths
    ...
    listing[address] = temp

pots = get_pots()
pot1_total = pots[0]["pot1"]["total"]
pot2_total = pots[1]["pot2"]["total"]

zip_growth = {'20746': (337320-211700)/(2117 * 4.5) - 7, '20001': 0, '20002': 0, '20003': (900000-670000)/(6700 * 4.5), '20004': 5, '20006': 4, '20007': 4, '20008': (755500 - 500000)/(5000 * 4.5), '20009': (720000 - 531000)/(5310*4.5), '20010': (-15), '20011': (6.4), '20012': (875000 - 600000)/(6000 * 4.5), '20015': (1200000 - 911000)/(9110*4.5), '20016': (1155000 - 652250)/(6522.5 * 4.5) - 6, '20017': (682500 - 490000)/(4900 * 4.5), '20018': (8.6), '20019': (450000 - 290000)/ (2900 * 4.5) - 4, '20020': (467000 - 285000)/(2850 * 4.5) - 7, '20024': (490000 - 321000)/(3210 * 4.5) - 5, '20032': (440000 - 295000)/(2950 * 4.5) - 0.5}



# predict which properties would be feasible
def predict(group):
    capital = pot1_total if group.lower() == "group1" else pot2_total
    ...
    for key in listing:
        if float(get_price(key)) <= capital:
            return_percent = percent_increase_valuation(key)
            print(return_percent)
            index = None

            if return_percent > addresses[0][1]:
                index = 0
            elif return_percent > addresses[1][1]:
                index = 1
            elif return_percent > addresses[2][1]:
                index = 2

            if index != None:
              addresses.insert(index, [key, return_percent])
              addresses = addresses[:3]

    while ["test", 0] in addresses:
        addresses.remove(["test", 0])
    ...
    return list #list of predicted good properties
...
def get_baths(address):
    return float(listing[address]["baths"])


def get_beds(address):
    return float(listing[address]["beds"])

def get_img_link(address):
    return listing[address]["image_link"]


def get_rental_income(address):
    return float(listing[address]["price"]) * 12/100


def break_even_time(address):
    income = get_rental_income(address)
    return float(get_price(address))//income


def percent_increase_valuation(address):
    zip = address[-5:]
    print("zip: ", zip)
    growth_rate = 4
    if zip in zip_growth:
        growth_rate = zip_growth[zip]

    print("growth rate: ",growth_rate)


    location_multiplier = 1.2
    property_multiplier = 1.8

    value = int(get_price(address)) * (1 + growth_rate/100 * location_multiplier * property_multiplier) ** 10

    print((value - int(get_price(address)))/int(get_price(address)) * 100)

    return (value - int(get_price(address)))/int(get_price(address)) * 100
